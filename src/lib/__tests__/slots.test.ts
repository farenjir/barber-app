...
describe('Slot calculation and validation', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  describe('Working hours validation', () => {
    it('should reject slots on closed days (Friday)', async () => {
      ...
      const available = await isSlotAvailable(friday, 45);
    ...
    it('should accept slots on open days within working hours', async () => {
      ...
      // Mock: Saturday is open 10:00-21:00
      (sql as any).mockResolvedValueOnce([{
        weekday: 6,
        start_time: '10:00',
        end_time: '21:00',
      ...
      const { isSlotAvailable } = await import('../slots');
      const available = await isSlotAvailable(saturday, 45);
    ...
    it('should reject slots outside working hours', async () => {
      ...
      // Mock: Working hours 10:00-21:00
      (sql as any).mockResolvedValueOnce([{
        weekday: 6,
        start_time: '10:00',
        end_time: '21:00',
      ...
      const { isSlotAvailable } = await import('../slots');
      const available = await isSlotAvailable(earlyMorning, 45);
    ...
    it('should reject slots that would extend past closing time', async () => {
      ...
      const available = await isSlotAvailable(lateSlot, 45);
  ...
  describe('Overlap detection', () => {
    it('should detect overlapping appointments', async () => {
      ...
      // Mock: Existing appointment at 14:30 (30 min duration)
      (sql as any).mockResolvedValueOnce([{
        id: 1,
      ...
      const available = await isSlotAvailable(newSlot, 45);
  ...
  describe('Duration fitting', () => {
    it('should fit exact duration in available time', async () => {
      ...
      const available = await isSlotAvailable(slot, 60);
  ...
  describe('Blocked slots', () => {
    it('should reject slots in blocked time ranges', async () => {
      ...
      // Mock: Blocked slot from 13:00 to 15:00
      (sql as any).mockResolvedValueOnce([{
        id: 1,
      ...
      const { isSlotAvailable } = await import('../slots');
      const available = await isSlotAvailable(slot, 45);
